...
async function readContent(pathname: string): Promise<{ data: Buffer; sha: string } | null> {
  ...
  if (!blobPayload.content || blobPayload.encoding !== "base64") throw new Error("GitHub database blob response was incomplete");
  return { data: Buffer.from(blobPayload.content.replace(/\n/g, ""), "base64"), sha: payload.sha };
}
...
function encrypt(snapshot: DatabaseSnapshot, key: Buffer): Buffer {
  ...
  const payload = Buffer.concat([Buffer.from("USDB1"), iv, cipher.getAuthTag(), encrypted]);
...
async function writeToGitHub(snapshot: DatabaseSnapshot): Promise<void> {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    if ((response.status === 403 || response.status === 429) && attempt < 2) {
      await new Promise((resolve) => setTimeout(resolve, Math.max(retryAfter * 1000, 1000)));
      continue;
    }
...
async function enqueuePersist(): Promise<void> {
  if (!state || !dirty) return;
  const snapshot = databaseSnapshot();
  dirty = false;
  persistQueue = persistQueue.then(() => writeToGitHub(snapshot)).catch((err) => {
    dirty = true;
...
export function scheduleGitHubPersist(): void {
  dirty = true;
...
export async function flushGitHubDatabase(reason = "manual"): Promise<void> {
  if (persistTimer) {
    clearTimeout(persistTimer);
    persistTimer = null;
  }
  logger.info({ reason }, "Flushing GitHub database");
...
export async function createProject(name: string, description: string | null): Promise<Project> {
  ...
  db().projects.unshift(project); scheduleGitHubPersist(); return project;
}
...
export async function deleteProject(id: number): Promise<boolean> {
  ...
  scheduleGitHubPersist(); return true;
}
...
export async function insertCollection(projectId: number, collection: string, data: Record<string, unknown>): Promise<CollectionRow> {
  ...
  db().collections.unshift(row); scheduleGitHubPersist(); return row;
}
...
export async function updateCollection(projectId: number, collection: string, id: number, data: Record<string, unknown>): Promise<CollectionRow | null> {
  ...
  if (!row) return null;
  row.data = data; row.updated_at = new Date().toISOString(); scheduleGitHubPersist(); return row;
}
...
export async function deleteCollection(projectId: number, collection: string, id: number): Promise<boolean> {
  ...
  db().collections.splice(index, 1); scheduleGitHubPersist(); return true;
}
...
export async function upsertGameCache(projectId: number, namespace: string, cacheKey: string, data: Record<string, unknown>, expiresAt: string | null): Promise<GameCacheRow> {
  ...
  if (existing) {
    existing.data = data; existing.expires_at = expiresAt; existing.updated_at = now; scheduleGitHubPersist(); return existing;
  }
  ...
  db().game_cache.unshift(row); scheduleGitHubPersist(); return row;
}
...
export async function deleteGameCache(projectId: number, namespace: string, cacheKey: string): Promise<boolean> {
  ...
  if (index < 0) return false;
  db().game_cache.splice(index, 1); scheduleGitHubPersist(); return true;
}
...
export function getStats(): { requestsToday: number; totalCollections: number; totalProjects: number; totalGameCacheEntries: number } {
  const today = new Date().toISOString().slice(0, 10);
  const requestsToday = db().request_logs.filter((row) => row.created_at.slice(0, 10) === today).length;
  const collectionKeys = new Set(db().collections.map((row) => `${row.project_id}:${row.collection}`));
  return { requestsToday, totalCollections: collectionKeys.size, totalProjects: db().projects.length, totalGameCacheEntries: db().game_cache.length };
}
export function insertLog(projectId: number, method: string, endpoint: string): void {
...
export async function restoreDatabase(snapshot: DatabaseSnapshot): Promise<void> { state = normalize(snapshot); await flushGitHubDatabase("restore"); }
export function githubDatabaseHealth(): { ok: boolean; latencyMs: number } { return state ? { ok: true, latencyMs: 0 } : { ok: false, latencyMs: 0 }; }
export function githubPersistenceStatus(): { dirty: boolean; lastPersistAt: string | null; lastPersistError: string | null } { return { dirty, lastPersistAt, lastPersistError }; }
