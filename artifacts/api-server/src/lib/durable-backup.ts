...
export async function createDurableBackup(): Promise<BackupManifest | null> {
  ...
  const snapshot = await pgExportSnapshot();
  const encrypted = encrypt(snapshot, c.key);
  const parentRef = await github(c.token, `/repos/${c.repo}/git/ref/heads/main`);
  ...
  for (let offset = 0; offset < encrypted.length; offset += CHUNK_BYTES) {
    ...
    const sha = await createBlob(c.token, c.repo, chunk);
    treeEntries.push({ path: `${BACKUP_PREFIX}/chunks/${String(chunkCount).padStart(6, "0")}.bin`, mode: "100644", type: "blob", sha });
    chunkCount++;
  }

  const firstTree = await github(c.token, `/repos/${c.repo}/git/trees`, { method: "POST", body: JSON.stringify({ base_tree: parentCommit.tree.sha, tree: treeEntries }) });
  const preliminary = { schemaVersion: 1 as const, createdAt: snapshot.createdAt, encrypted: true as const, algorithm: "aes-256-gcm" as const, compression: "gzip" as const, chunkCount, bytes: encrypted.length };
  const manifestSha = await createBlob(c.token, c.repo, Buffer.from(JSON.stringify(preliminary, null, 2)));
  const finalTree = await github(c.token, `/repos/${c.repo}/git/trees`, { method: "POST", body: JSON.stringify({ base_tree: firstTree.sha, tree: [{ path: `${BACKUP_PREFIX}/latest.json`, mode: "100644", type: "blob", sha: manifestSha }] }) });
  const commit = await github(c.token, `/repos/${c.repo}/git/commits`, { method: "POST", body: JSON.stringify({ message: `chore(backup): universal-server snapshot ${snapshot.createdAt}`, tree: finalTree.sha, parents: [parentSha] }) });
  await github(c.token, `/repos/${c.repo}/git/refs/heads/main`, { method: "PATCH", body: JSON.stringify({ sha: commit.sha }) });

  const result: BackupManifest = { ...preliminary, commitSha: commit.sha };
  logger.info({ commitSha: commit.sha, bytes: encrypted.length, chunks: chunkCount }, "Durable backup completed");
  return result;
}

async function getContent(token: string, repo: string, path: string, ref = "main"): Promise<Buffer> {
  const file = await github(token, `/repos/${repo}/contents/${path}?ref=${encodeURIComponent(ref)}`);
  return Buffer.from(String(file.content).replace(/\n/g, ""), "base64");
}

export async function restoreLatestBackupIfNeeded(): Promise<boolean> {
  const c = config();
  if (!c || await pgHasPersistentData()) return false;
  try {
    ...
    for (let i = 0; i < manifest.chunkCount; i++) {
      chunks.push(await getContent(c.token, c.repo, `${BACKUP_PREFIX}/chunks/${String(i).padStart(6, "0")}.bin`, manifest.commitSha));
    }
    const snapshot = decrypt(Buffer.concat(chunks), c.key);
    await pgRestoreSnapshot(snapshot);
    sqReplaceFromSnapshot(snapshot);
    logger.info({ createdAt: snapshot.createdAt, commitSha: manifest.commitSha }, "Durable backup restored");
    return true;
  } catch (err) {
    logger.error({ err }, "Durable backup restore failed");
    return false;
  }
}
